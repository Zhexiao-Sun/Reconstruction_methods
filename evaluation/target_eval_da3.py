#!/usr/bin/env python3
"""
Evaluate World Models Performance using Depth Anything 3 (image-only inference).
"""
...
def evaluate_world_models(
    ...
    if output_dir is None:
        output_dir = os.path.join(PROJECT_ROOT, "evaluation_results/da3")
    if da3_config_path is None:
        da3_config_path = os.path.join(PROJECT_ROOT, "pipelines/da3/configs/da3_trajectory_config.yml")
    if eval_config_path is None:
        eval_config_path = os.path.join(PROJECT_ROOT, "evaluation/config/vggt_config.yaml")

    print("=" * 80)
    print("World Models Evaluation using Depth Anything 3 (Image-Only)")
    print("=" * 80)
    ...
    da3_cfg = load_da3_config(da3_config_path)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print("\n" + "=" * 80)
    print("Loading Depth Anything 3 Model (one-time setup)")
    print("=" * 80)
    model = setup_model(da3_cfg["model"]["model_id"], device)
    ...
    for model_name, model_videos in wm_videos.items():
        ...
        for segment_dir in tqdm(segment_dirs, desc=f"Processing {model_name}"):
            ...
            try:
                ...
                desired_num_frames = da3_cfg["model"].get("num_frames")
                existing_frames = glob.glob(os.path.join(frame_dir, "frame_*.jpg"))
                needs_refresh = not existing_frames
                if desired_num_frames is not None:
                    needs_refresh = needs_refresh or len(existing_frames) != int(desired_num_frames)
                if needs_refresh:
                    ...
                    extract_video_frames(
                        wm_video_path,
                        frame_dir,
                        frame_stride=da3_cfg["model"].get("frame_stride", 1),
                        max_frames=da3_cfg["model"].get("max_frames"),
                        num_frames=desired_num_frames,
                    )

                prediction = run_da3_inference(frame_dir, model, da3_cfg["model"])
                ...
                apply_scale = bool(da3_cfg["model"].get("apply_scale_factor", True))
                ...
                trajectory, orientations, trajectory_2d = extract_trajectory_from_extrinsics(
                    ...
                    axis_order=da3_cfg.get("axis_mapping", {}).get("order"),
                )
    ...
    for segment_name, segment_results in all_results["by_segment"].items():
        for model_name, result in segment_results.items():
            ...
            row = {
                ...
                "is_metric": result.get("is_metric"),
                "scale_applied": result.get("scale_applied"),
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate World Models Performance using Depth Anything 3")
    ...
    parser.add_argument("--da3_config_path", type=str, default=None, help="Path to DA3 configuration file")
    ...
    results = evaluate_world_models(
        ...
        da3_config_path=args.da3_config_path,
    ...
    print("=" * 80)
    output_dir_to_show = args.output_dir if args.output_dir else os.path.join(PROJECT_ROOT, "evaluation_results/da3")
