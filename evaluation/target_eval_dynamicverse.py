#!/usr/bin/env python3
"""
Evaluate World Models Performance using DynamicVerse (image-only inference).
"""
...
sys.path.append(os.path.join(PROJECT_ROOT, "pipelines/dynamicverse"))
sys.path.append(os.path.join(PROJECT_ROOT, "evaluation"))

from extrinsic_path.extract_extrinsic_path import (
    load_config as load_dynamicverse_config,
...
def evaluate_world_models(
    ...
    if output_dir is None:
        output_dir = os.path.join(PROJECT_ROOT, "evaluation_results/dynamicverse")
    if pipeline_config_path is None:
        pipeline_config_path = os.path.join(
            PROJECT_ROOT, "pipelines/dynamicverse/configs/dynamicverse_trajectory_config.yml"
        )
    if eval_config_path is None:
        eval_config_path = os.path.join(PROJECT_ROOT, "evaluation/config/vggt_config.yaml")

    print("=" * 80)
    print("World Models Evaluation using DynamicVerse (Image-Only)")
    print("=" * 80)
    ...
    pipeline_cfg = load_dynamicverse_config(pipeline_config_path)
    pipeline_output_dir = pipeline_cfg["paths"]["output_base_path"]
    if not os.path.isabs(pipeline_output_dir):
        pipeline_output_dir = os.path.join(PROJECT_ROOT, pipeline_output_dir)
    ...
    for model_name, model_videos in wm_videos.items():
        ...
        for segment_dir in tqdm(segment_dirs, desc=f"Processing {model_name}"):
            ...
            try:
                _, _, trajectory_2d, info = process_single_video(
                    wm_video_path,
                    pipeline_cfg,
                    pipeline_output_dir,
                    segments_dir=segments_dir,
                )
                trajectory_length_meters = float(info.get("trajectory_length_meters", 0.0))
...
def create_evaluation_visualizations(all_results, output_dir, timestamp):
    ...
    for segment_name, segment_results in all_results["by_segment"].items():
        ...
        ax.set_title(f"Trajectory Comparison\nDynamicVerse - {segment_name}", fontsize=36, pad=20)
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate World Models Performance using DynamicVerse (Image-Only)"
    )
    ...
    print("=" * 80)
    output_dir_to_show = args.output_dir if args.output_dir else os.path.join(PROJECT_ROOT, "evaluation_results/dynamicverse")
