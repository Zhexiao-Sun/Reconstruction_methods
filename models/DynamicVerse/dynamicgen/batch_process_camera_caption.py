...
def main():
    ...
    # Iterate through all video files in video_dir
    ...
    for root, _, files in os.walk(args.video_dir):
        for fname in files:
            ...
            if ext.lower() in supported_exts and basename != "masks" and basename != "original" and basename != "overlay" and basename != "segmented":
                rel_path = os.path.relpath(os.path.join(root, fname), args.video_dir)
                data.append({
                    "video": rel_path,
                    "question": "Describe the camera movement in this video in detail, at least 300 words. The following 5 aspects need to be included:1. Shot type: e.g., wide shot, close-up, medium shot, over-the-shoulder 2. Angle: e.g., high angle, low angle, eye level, bird,s-eye view 3. Movement: e.g., pan, tilt, dolly in, dolly out, truck, crane, handheld 4. Transition: e.g., cut, fade in, fade out, dissolve, wipe, match cut 5. Special effects: list any visual or post-production effects (e.g., slow motion, time-lapse, lens flare, CGI).Only camera motion needs to be analyzed, not scene analysis.",
                    "answer": ""
                })
    ...
    if not data:
        print(f"Error: No video files found in {args.video_dir}. Exiting.")
        return

    print(f"Found {len(data)} video files in {args.video_dir}")
